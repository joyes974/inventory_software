...
<div class="portlet-content"><select name="main_category" class="main_category1" onchange="change(this.value)">
...
<form id="add_sale" method='POST' action='<?=site_url('sale/add_sale');?>' enctype="multipart/form-data">
 Date: <input type='text' class="date_picker" name='date' value="<?=$_POST['date']?>"/><?=form_error('date','<span>','</span>')?>
 Sl No: <input type='text' style="color:green;" name='serial_id' readonly value="<?php  echo $this->sale_model->get_sl()?>"/>
 Order No: <input type='text' class="" name='order_no' readonly value="<?php  echo $this->sale_model->get_sl()?>"/>
 Do no.<input type='text' class="number" name='do_no' value="<?php  echo $this->sale_model->get_sl()?>"/></div>
<div class="form_content portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" style="width:900px;margin-top:50px;">
   ...
   <div class="portlet-content fl_left" style="width: 40%;border:1px ridge grey;height:250px;">
       ...
       <input type="hidden" name="reason"/>
            <tr>
                <th> Invoice No:</th>
                <td><input type='text' style="color:green;" class="input_txt width_150 " name='invoice_no'  readonly value="<?php  echo $this->sale_model->get_invoice()?>"/><?=form_error('invoice_no','<span>','</span>')?></td>
            </tr>
            <tr>
                <th> Amount:</th>
    ...
    <div class="portlet-content fl_left" style="width: 40%;border:1px solid grey;margin:10px;padding:10px;"><? echo $res['category_name']; ?><table> <tbody><tr>
                <th><?=$poultry_products[$i]['product_name']?></th><?='(K.G.)'?>
                <td><input class="input_txt width_100 number" name="quantity[]" type="text">(K.G.)</td>
